...
function randomPoints(number, geojson) {
    const points = [];
    for (let i = 0; i < number; i++) {
        const point = geojson.features[Math.floor(Math.random() * geojson.features.length)];
        points.push(point);
    }
    return points;
}
...
function getCoordinates(point) {
    return [point.properties.LABEL_Y, point.properties.LABEL_X];
}
